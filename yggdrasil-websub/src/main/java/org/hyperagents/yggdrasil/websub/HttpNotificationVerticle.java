...
public class HttpNotificationVerticle extends AbstractVerticle {
  ...
  private final int OBS_PROPERTY_UPDATE_MAX_RETRIES = 5;
  ...
  public void start() {
    ...
    ownMessagebox.receiveMessages(message -> {
      switch (message.body()) {
        ...
        case HttpNotificationDispatcherMessage.RemoveCallback(String requestIri, String callbackIri) ->
        ...
        case HttpNotificationDispatcherMessage.EntityDeleted m -> {
          ...
          this.registry.getCallbackIris(entityIri)
              .forEach(c -> this.createNotificationRequest(client, webSubHubUri, c, entityIri)
                  .send(this.reponseHandler(c)));
        }
        case HttpNotificationDispatcherMessage.ArtifactObsPropertyUpdated(String requestIri, String content) ->
          this.handleObsPropertyUpdateSending(
              client,
              webSubHubUri,
              requestIri,
              Pattern.compile(
                  ...
                  .matcher(content)
                  .replaceAll(r -> content.charAt(r.start() - 1) == '"' && content.charAt(r.end()) == '"'
                      ? r.group()
                      : "\"" + r.group() + "\""));
  ...
  private void handleObsPropertyUpdateSending(
      final WebClient client,
      final String webSubHubUri,
      final String requestIri,
      final String content) {
    this.registry.getCallbackIris(requestIri)
        .forEach(c -> this.createNotificationRequest(client, webSubHubUri, c, requestIri)
            .putHeader(HttpHeaders.CONTENT_LENGTH, Integer.toString(content.length()))
            .sendBuffer(Buffer.buffer(content), this.obsPropertyUpdateReponseHandler(client, webSubHubUri, c,
                requestIri, content, OBS_PROPERTY_UPDATE_MAX_RETRIES, 0)));
  }
  ...
  private Handler<AsyncResult<HttpResponse<Buffer>>> obsPropertyUpdateReponseHandler(
      final WebClient client,
      final String webSubHubUri,
      final String callbackIri,
      final String requestIri,
    ...
    return ar -> {
      if (ar.succeeded()) {
        ...
        if (response.statusCode() == HttpStatus.SC_OK) {
          LOGGER.info(
              "OBS Notification sent to: "
                  + callbackIri
                  + ", status code: "
                  + response.statusCode());
        } else {
          LOGGER.info(
              "OBS Failed to send notification to: "
                  + callbackIri
                  + ", status code: "
                  + response.statusCode());
          retry(client, webSubHubUri, callbackIri, requestIri, content, maxRetries, retryCount + 1);
        }
      } else {
        LOGGER.info(
            "OBS Failed to send notification to: "
  ...
  private void retry(
    ...
    if (retryCount < maxRetries) {
      long delay = (long) Math.pow(2, retryCount) * 1000; // Exponential backoff with base 2, delay in milliseconds

      vertx.setTimer(delay, id -> {
        this.createNotificationRequest(client, webSubHubUri, callbackIri, requestIri)
            .putHeader(HttpHeaders.CONTENT_LENGTH, Integer.toString(content.length()))
            .sendBuffer(Buffer.buffer(content),
                obsPropertyUpdateReponseHandler(client, webSubHubUri, callbackIri, requestIri, content, maxRetries,
                    retryCount + 1));
      });
    } else {
      System.out.println("Max retries reached for: " + callbackIri);
    }
  }
